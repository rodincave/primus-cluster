...
Adapter.prototype.broadcast = function broadcast(data, opts, clients, cb) {
  ...
  if (rooms.length) {
    ...
    multi.exec(function (err, replies) {
      ...
      replies.forEach(function (roomIds) { // a loop on the roomIds
        //console.log("people in room :", roomIds);
        if (tolength) {
          //console.log("roomsIds", roomIds);
          // do the same as in the normal adapter ?
          for (var j=0; j<tolength; j++) {
            var recieverid = to[j];
            //console.log("clients", clients);
            if (roomIds.indexOf(recieverid) < 0 || ids[recieverid]) continue;
            //console.log("ok send...");
            if (!clients[recieverid]) continue;
            //console.log("sending message...", method);
            //console.log("client", clients[recieverid])
            clients[recieverid][method].apply(clients[recieverid], data);
            ids[recieverid] = true;
          }
        } else {
    ...
    this.client.keys('socket:*', function (err, sockets) {
      ...
      if (tolength) {
        ...
        for (var j=0; j<tolength; j++) {
          var recieverid = to[j];
          if (ids.indexOf(recieverid) < 0) continue;
          clients[recieverid][method].apply(clients[recieverid], data);
          ids[recieverid] = true;
        }
      } else {
